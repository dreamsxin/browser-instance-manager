...
class WebScraper {
  constructor(options = {}) {
    this.redisSortedSet = new RedisSortedSet({
      url: options.redisUrl || process.env.REDIS_URL || 'redis://127.0.0.1:6379',
      password: options.redisPassword || process.env.REDIS_PASSWORD || undefined,
      database: options.redisDatabase || process.env.REDIS_DATABASE || undefined
    });
    ...
    this.maxRequestsBeforeRestart =
      ...
      parseInt(process.env.MAX_REQUESTS_BEFORE_RESTART) ||
      500;
    this.maxPageUsage =
      options.maxPageUsage || parseInt(process.env.MAX_PAGE_USAGE) || 20;
    this.initialPagePoolSize =
      options.initialPagePoolSize ||
      parseInt(process.env.INITIAL_PAGE_POOL_SIZE) ||
      5;
  ...
  async initializePagePool() {
    ...
    console.log(
      `Page pool initialized with ${this.pagePool.length} pages (target: ${this.initialPagePoolSize})`
    );
  }
  ...
  async waitForAllPagesToComplete(timeoutMs = 30000) {
    ...
    while (Date.now() - startTime < timeoutMs) {
      const inUsePages = this.pagePool.filter(
        (p) => this.pageStatus.get(p.page) === "in-use"
      );
  ...
  async checkAndRestartBrowser() {
    this.requestCount++;
    console.log(
      `Request count: ${this.requestCount}/${this.maxRequestsBeforeRestart}`
    );

    if (
      this.requestCount >= this.maxRequestsBeforeRestart &&
      !this.browserRestartInProgress
    ) {
      console.log(
        `Reached ${this.maxRequestsBeforeRestart} requests, queuing browser restart...`
      );
  ...
  async scrapePage(word, options = {}) {
    ...
    if (this.browserRestartInProgress && this.restartPromise) {
      console.log(
        `Browser restart in progress, waiting for restart to complete for: ${word}`
      );
      try {
  ...
  // 获取当前状态信息
  getStatus() {
    const available = this.pagePool.filter(
      (p) => this.pageStatus.get(p.page) === "available"
    ).length;
    const inUse = this.pagePool.filter(
      (p) => this.pageStatus.get(p.page) === "in-use"
    ).length;
